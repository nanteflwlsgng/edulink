...
export default function StudentDashboard() {
        ...
        const fetchDashboardData = async () => {
            ...
            try {
                ...
                try {
                    ...
                    // 1. On sécurise l'accès aux données (souvent dans .data.data)
                    const rawNotifs = notifRes.data?.data?.notifications || notifRes.data?.data || [];

                    // 2. On formate pour le frontend
                    const formattedNotifs = Array.isArray(rawNotifs) ? rawNotifs.map(notif => ({
                        id: notif.id_notification || notif.id, // Fallback si le nom change
                        title: notif.titre,
                        message: notif.message,

                        // Mapping des types Backend (SUCCESS/ERROR/INFO) -> Frontend (success/error/info)
                        type: notif.type === 'SUCCESS' ? 'success' :
                            notif.type === 'ERROR' ? 'error' : 'info',

                        date: new Date(notif.date_creation).toLocaleDateString('fr-FR', {
                            day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit'
                        }),

                        // On garde l'état lu/non lu pour l'affichage (point bleu)
                        lue: notif.lue
                    })) : [];
                    ...
                    console.warn("Info: Impossible de charger les notifications", e);
                    setNotifications([]); // Évite que l'app plante si erreur
                }
                ...
                try {
                    ...
                    if (appsRes.data.success) {
                        const formattedApps = appsRes.data.data.map(app => {
                            ...
                            // 1. Récupération et nettoyage du statut
                            const raw = (app.statut || "").toString().toUpperCase();

                            // 2. Mapping vers l'interface
                            let uiStatus = 'En attente';

                            if (raw === 'VALIDEE' || raw === 'ADMIS' || raw === 'VALIDE') {
                                uiStatus = 'Admis';
                            }
                            else if (raw === 'ANNULEE' || raw === 'REFUSE') {
                                uiStatus = 'Refusé';
                            }
                            else if (raw === 'INSCRIT') {
                                uiStatus = 'Inscrit';
                            ...
                            return {
                                ...
                                status: uiStatus, // Contiendra 'Admis' si validé

                                date: app.date_inscription ? new Date(app.date_inscription).toLocaleDateString('fr-FR') : "-",
                                priceDisplay: app.formation?.prix ? `${app.formation.prix.toLocaleString()} Ar` : "Non spécifié",
                                price: app.formation?.prix || 0,
                                logo: app.formation?.ecole?.logo || null,
                                enrollmentComplete: uiStatus === "Inscrit"
                            };
                        });
    ...
    const handlePaymentClick = (application) => {
        // On redirige vers la route /paiement/ID
        // On passe les infos (prix, titre...) dans 'state' pour les afficher tout de suite
        navigate(`/paiement/${application.id}`, {
            state: { application: application }
        });
    };
    const handleDownloadCard = async (inscriptionId) => {
    try {
        // On affiche un petit loading ou toast si tu veux
        // showToast("info", "Génération de la carte...");

        // Appel API avec responseType: 'blob' (Indispensable pour les fichiers)
        const response = await api.get(`/etudiants/${inscriptionId}/carte`, {
            responseType: 'blob' 
        });

        // Création de l'URL pour le navigateur
                            ...
                            <div className="space-y-3">
                                {applications.length === 0 ? (
                                    ...
                                    applications.map((app) => (
                                        <div key={app.id} onClick={() => openAppModal(app)} className="group flex justify-between p-5 rounded-2xl bg-gray-50 hover:bg-white border border-transparent hover:shadow-lg cursor-pointer transition-all duration-300 items-center">

                                            {/* INFO GAUCHE (Logo + Texte) */}
                                            <div className="flex items-center gap-4">
                                                <div className="w-12 h-12 rounded-xl bg-white flex items-center justify-center font-bold text-[#370669] shadow-sm border border-gray-100 overflow-hidden">
                                                    {app.logo ? <img src={app.logo} className="w-full h-full object-cover" /> : (app.school ? app.school.charAt(0).toUpperCase() : "?")}
                                                </div>
                                                <div>
                                            ...
                                            {/* ACTION DROITE (Bouton ou Badge) */}
                                            <div className="flex items-center gap-3">

                                                {/* CAS 1 : ADMIS -> Bouton Payer */}
                                                {app.status === 'Admis' ? (
                                                    <button
                                                        onClick={(e) => {
                                                            e.stopPropagation();
                                                            handlePaymentClick(app);
                                                        }}
                                                        className="bg-[#18B49C] text-white px-5 py-2 rounded-full text-xs font-bold hover:bg-[#149c86] shadow-md transition-transform hover:scale-105 flex items-center gap-2 animate-pulse-slow"
                                                    >
                                                        <CreditCard className="w-3.5 h-3.5" /> Payer
                                                    </button>
                                                ) : app.status === 'Inscrit' ? (
                                                    // 🔥 CAS 2 : INSCRIT -> Bouton Télécharger Carte (NOUVEAU)
                                                    <button
                                                        onClick={(e) => {
                                                            e.stopPropagation();
                                                            handleDownloadCard(app.id); // On appelle la fonction de téléchargement avec l'ID inscription
                                                        }}
                                                        className="bg-[#370669] text-white px-5 py-2 rounded-full text-xs font-bold hover:bg-[#2a0450] shadow-md transition-transform hover:scale-105 flex items-center gap-2"
                                                    >
                                                        <Download className="w-3.5 h-3.5" /> Ma Carte
                                                    </button>
                                                ) : (
                                                    // CAS 3 : AUTRES STATUTS (Badge simple)
                                                    <span className={`px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider border ${app.status === 'Refusé' ? 'bg-red-50 text-red-600 border-red-100' :
                                                            'bg-blue-50 text-blue-600 border-blue-100'
                                                        }`}>
                                                        {['En attente', 'Vu', 'PAYE', 'VALIDEE'].includes(app.status) ? (app.status === 'PAYE' || app.status === 'VALIDEE' ? "EN COURS..." : "ENVOYÉE") : app.status}
                                                    </span>
                                                )}
                                            </div>
                                        </div>
...
function NotificationsModal({ notifications, onClose }) {
    return (
        ...
        >
            <div 
                className="bg-white w-full max-w-md rounded-[2rem] p-6 shadow-2xl relative animate-scaleIn" 
                onClick={e => e.stopPropagation()}
            >
                {/* HEADER DU MODAL */}
                <div className="flex justify-between items-center mb-6 pb-4 border-b border-gray-100">
                    <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
                        <Bell className="w-6 h-6 text-[#27b6d8] fill-current" /> Alertes
                    </h2>
