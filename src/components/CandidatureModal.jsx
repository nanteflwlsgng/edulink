...
import { 
  ...
  FileText, Download, CheckCircle, Eye, XCircle, Bell, ChevronRight
} from "lucide-react";
...
export default function CandidatureModal({ isOpen, onClose, candidat, onAction }) {
  ...
  // Simulation de documents (Tout est complet maintenant)
  const documents = [
    { name: "Curriculum Vitae", file: "cv_candidat.pdf", size: "1.2 Mo" },
    { name: "Lettre de Motivation", file: "lettre_motiv.pdf", size: "800 Ko" },
    { name: "Relevé de notes", file: "notes_bac.pdf", size: "2.4 Mo" },
    { name: "Pièce d'identité", file: "cin_scan.jpg", size: "1.5 Mo" },
    { name: "Photo d'identité", file: "photo.jpg", size: "500 Ko" },
  ];
            ...
            <div className="text-white">
                ...
                <div className="flex flex-wrap items-center gap-4 text-white/80 text-sm">
                    ...
                    <span className="flex items-center gap-1.5">
                        <Calendar className="w-4 h-4" /> Reçu le {candidat.date}
                    </span>
                </div>
            </div>
        </div>
                    ...
                    {activeTab === "info" ? (
                        <div className="space-y-6 animate-fadeIn">
                             <div className="grid grid-cols-2 gap-4">
                                <InfoItem icon={Mail} label="Email" value="jean.dupont@student.com" />
                                <InfoItem icon={Phone} label="Téléphone" value="+261 34 00 000 00" />
                                <InfoItem icon={Calendar} label="Né(e) le" value="12 Mai 2002 (23 ans)" />
                                <InfoItem icon={User} label="Sexe" value="Masculin" />
                                <InfoItem icon={MapPin} label="Ville" value="Antananarivo" />
                                <InfoItem icon={GraduationCap} label="Dernier Diplôme" value="Licence Informatique" />
                             </div>
                             ...
                             <div className="bg-gray-50 p-6 rounded-2xl border border-gray-100">
                                ...
                                <p className="text-sm text-slate-700 leading-relaxed italic">
                                    "Passionné par le développement web depuis mon plus jeune âge, je souhaite intégrer votre Master pour approfondir mes connaissances en architecture logicielle. J'ai réalisé plusieurs projets personnels..."
                                </p>
                             </div>
                        </div>
                    ) : (
                    ...
                    <div className="sticky top-0 space-y-6">
                        ...
                        <div className="pt-4">
                            <button className="w-full text-xs font-bold text-gray-400 hover:text-[#370669] flex items-center justify-center gap-2 transition-colors group">
                                <Mail className="w-3.5 h-3.5 group-hover:scale-110 transition-transform" /> Envoyer un message direct
                            </button>
                        </div>
                    </div>
                </div>
