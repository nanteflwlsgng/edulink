...
export const getDashboardStats = async (req, res) => {
  try {
    const stats = await ecoleService.getStatistiquesEcole(req.user.id_utilisateur);
...
export const getCandidatures = async (req, res) => {
  try {
    const { status } = req.query; // ?status=Admis ou ?status=En attente
    const result = await ecoleService.getCandidatures(req.user.id_utilisateur, status);
    res.json({ success: true, data: result });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
export const traiterCandidature = async (req, res) => {
  try {
    const { id } = req.params; // id_inscription
    const { decision } = req.body; // 'VALIDEE' ou 'ANNULEE'
    
    await ecoleService.traiterCandidature(req.user.id_utilisateur, parseInt(id), decision);
    res.json({ success: true, message: "Candidature traitée" });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
export const getFinances = async (req, res) => {
  try {
    const result = await ecoleService.getPaiementsEcole(req.user.id_utilisateur);
    res.json({ success: true, data: result });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
...
export const validerPaiement = async (req, res) => {
  try {
    const { id } = req.params; // id_paiement
    await ecoleService.validerPaiement(req.user.id_utilisateur, parseInt(id));
